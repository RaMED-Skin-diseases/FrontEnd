...
export default function SavedPostsScreen({ navigation }) {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://new-env.eba-6dsh89vt.eu-north-1.elasticbeanstalk.com/community/`;
  ...
  const toggleSavePost = async (postId) => {
    try {
      ...
      const response = await fetch(
        `http://new-env.eba-6dsh89vt.eu-north-1.elasticbeanstalk.com/community/save-post/${postId}/`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
